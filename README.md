...
* **对弈模式**（2026-09）：`模式` 下拉框决定**这一场到底学不学** ——
  **训练对局**（双方各自学习，默认）、**评估对局**（冻结 B 方、只让 A 方学，比分变化才
  能归因到 A 自己）、**只对弈不学习**（两条学习路径都关掉，比分可直接比较）。
  为什么需要它：在此之前"学不学"没有任何权威开关 —— 实测（`test_match [2.7c]/[2.7d]`）
  `预训步数 = 0` 只关得住 PPO 那类 agent 的**第一条**学习路径，而 SAC+AZ 在 `selectMove`
  里还有第二条（`learnFromSearch`），那个勾选框**压根管不到**（关掉预训后它仍每手更新）。
  对局报告里会写明本场用的模式，并在"不学习"的模式下提示**不能当作棋力结论**。
* **"不学"必须包含"权重不变"**（P0-b，2026-09）：模式只管住"这一手学不学"是不够的 ——
  评估/只对弈模式声称冻结，而权重仍会从三条缝里被改掉：
  1. **后台训练线程**（开机就在跑，每轮把新权重同步回主 agent）。现在非训练模式下它
     **整体停摆**（训练线程每轮开头 + 写权重之前各判一次，见 `isBackgroundTrainingPaused`），
     于是**人机对战那条路**（不在 `matchAgents` 里）与**局与局之间**也覆盖到了；
  2. **A 与 B 是同一个 agent 类型**时它们本来就是**同一个常驻实例** ⇒ A 每学一次，B 用的
     那份权重当场就变。现在评估模式开场会取一份 B 的**权重快照**并用它单独建一个冻结实例，
     B 那一手走这个实例（可选的逐字节审计会把它与开场快照比对后写进报告）；
  3. 人机对战里 AI 作为"冻结的对手"不再学习（AI 执黑，人 = 学习者）。
  对局报告里会写明"B 方冻结口径"（快照 / 独立实例 / 无可训练权重）与后台训练跳过了几轮 ——
  读数自带前提，不用读者自己悟。`test_match [2.7j]/[2.7k]` 钉住（含一个**对照**：
  把快照关掉，那条"权重当场就变"的洞会原样复现）。
* **对手入训**（P1，2026-09，默认关闭）：决定**探索里的"对手那一半"由谁产生**。
  在此之前它由学习方**自己的策略**采样（纯自对弈）—— 于是学到的永远是"怎么跟自己下"，
  真实对手（哪怕选了 Alpha-Beta 当陪练）的着法**从未参与数据生成**。
  打开后学习方每手决策前的探索会去问**真实对手**"在这个局面上你会怎么走"
  （走的是它对局时同一条决策代码），对手的着法**只推进局面、不记成学习方的样本**
  （记了就变成"off-policy 却打着 on-policy 标签"，对 REINFORCE/PPO 是实打实的偏差），
  对手**被问到时也不许学习**。⚠ 问一次对手 = 一次完整决策（SAC+AZ 实测 2.3 s/手），
  所以有手数上限（默认 1 = 只问对手对第一步的应手）；⚠ 打开后价值/策略目标
  **条件化于这个对手**，适合"专门练一个对手"，不适合常开。`test_pretrain`（探索内部三条
  语义）与 `test_match [2.7l]`（对弈里"真的问了 / 对手没偷学 / A 照常学"）钉住。
...
* **后台持续训练**（2026-09 补齐）：当前选中的 agent 在后台线程里持续"自对弈 → 在线更新 →
  ...
  **评估/只对弈模式下它整体停摆**（P0-b：那两种模式声称"冻结"，而冻结必须包含权重不变 ——
  否则报告里那句"双方都不学"会被读成"权重也不变"，见 `test_match [2.7k]`）。
