...
**状态编码用规范视角 14×90 = 1260 维**（与 EVAB 同一约定：7 类棋子 × {己方, 对方} 的 one-hot 平面，轮到黑方时 `x → 9-x` 镜像）。这样"轮到谁走"被编码进视角，同一个网络对红黑双方都是"我方的子在自己下方"。本项目其它 RL agent（PG/DQN/PPOMCTS/DQNMCTS）用的是 90 维**非**规范编码，价值函数的"谁走"是隐含的 —— 对 AlphaZero 这种"策略/价值都从走棋方视角定义"的算法，规范视角是必需的。
...
| **每局环境奖励** | `ChessBoard::gameRewardSample`（每局结束） | 每场对弈两条（A / B），每局各一个点，**走子方视角**（吃子 + 终局 ±1） |
...
- 曲线下面是"数字读数"标签（`损失 ... 最新 0.048, 均值 0.159, 51 点`）。这不只是为了
  ...
  断言"曲线真的有数据"**（见 13.4）。
- 哪些 agent 上报损失：`AgentBase::getLastTrainLoss()`，默认返回 NaN（不上报）。已实现：
  `SACAZAgent`（critic 的 MSE）、`DQNAgent`（平均平方 TD 误差）、`EVABAgent`（价值网
  蒸馏的 MAE）。PG / PPO+MCTS / DQN+MCTS 的训练循环里没有 scalar loss，保持"不上报"
  （曲线里没有点，不画假线）。
- 支持"清空曲线"与"导出 CSV"。
...
### 15.4 顺带的一个优化：这个 15 秒不该花在启动上

那 3 × 146 MB 只有在用户真的选稀疏 MoE 变体时才需要，而绝大多数会话用不到。所以
`startupLoad()` **不再预加载它**（路径仍然登记在 `s_weightPaths` 里），改由第一次使用时
懒加载 + 弹沙漏：

```
启动耗时: 19 s -> 1.7 s     （实测, verify_busy_ui.ps1）
首次使用稀疏 MoE 变体: 弹沙漏 ~15 s, 然后正常走子（verify_busy_lazy.ps1）
```
