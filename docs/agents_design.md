...
>
> ### ⚠ 2026-09 第五轮：还原版改为**独立类**（不再继承）+ 新增它的 **TB 专家骨干**那一支
>
> 用户口径（承第三轮）："以后任何对 `SACAZAgent` 的默认值或实现改动都**不可能**渗进还原版"。
> 第三轮那个"薄派生类"做不到这一点 —— 凡是没有被**显式钉住**的量都会跟着基类默认值漂走
> （F1 那一轮就漂过一次：目标网同步率）。于是 `SACAZLegacyAgent` 改成**不继承**的独立类，
> 自带一份 SAC 实现（`src/sacazlegacyagent.h` + `.cpp`：与 `sacazagent.cpp` 是刻意的 1:1
> 拷贝 + 59e5233 口径）。所以第三轮那段里"派生类 / 头文件实现、无 .cpp"的说法**已作废**；
> 头文件末尾有一条 `static_assert(!std::is_base_of<SACAZAgent, SACAZLegacyAgent>::value)`
> 把"不许改回继承"从注释变成编译错误。
>
> 随后又要求"**增加一个使用 TB 专家骨干的还原版 agent**"：口径一字不改，只换骨干。
> 界面因此多了一项 **"SAC+MCTS+AlphaZero (59e5233 还原版, 稀疏MoE+TB专家)"** =
> `ChessBoard::AGENT_SACAZ_OLD_MOE` = 同一个类 + `Backbone::SparseMoeTb`
> （与 `AGENT_SACAZ_MOE` 同骨干）。这样"换骨干值多少"与"口径差别值多少"是两个**分开**的
> 实验，而不是混在一次对比里（本仓库在这上面栽过一次，见 §13 那一类教训）。
>
> | 项 | `AGENT_SACAZ_OLD` | `AGENT_SACAZ_OLD_MOE` |
> |---|---|---|
> | 骨干 | `Backbone::Mlp` | **`Backbone::SparseMoeTb`**（E=4 top-1；expertHidden 64；aux 0.1） |
> | 口径 | 59e5233：目标熵 0.98 / α lr 1e-3 / 目标不夹 + 纯 MSE / 叶子全量 / tau=1e-3 每 64 步 | **完全相同**（硬编码在类里，不随骨干变） |
> | 界面模拟次数 | 256（`SACAZ_SIMS`） | **16**（`SACAZ_MOE_SIMS`：TB 一次模拟 ~10.9 ms ≈ 175 ms/手） |
> | 后台训练模拟次数 | `BG_TRAIN_SACAZ_SIMS` | `BG_TRAIN_SACAZ_MOE_SIMS`（64） |
> | 权重前缀 | `weights/sacaz_old_agent` | **`weights/sacaz_old_moe_agent`** |
> | 临时前缀 | `weights/_temp_train_sacaz_old` | `weights/_temp_train_sacaz_old_moe` |
>
> 前缀的**单一来源**是 `SACAZLegacyAgent::defaultWeightPrefix(Backbone)`（无参版本 = MLP
> 那一支，保留给既有调用方），身份标签同理按骨干分支（`guiAgentLabel()`）—— 两支的参数量 /
> 每模拟耗时 / MoE 使用直方图差很多，身份不写在自检第一行就会把读数记到同一本账上。
> 本类**仍然不含**奖励塑形与任何 critic 抑制：那一批开关在它身上**连成员都没有**
> （`test_sacaz` [14] 用 SFINAE 探针逐个钉住，而不是"默认关着"）。
...
```
<Agent>::selfCheckReport()                // 只读, 副本 + 局部解码 (十六个 agent 全部实现)
...
* **契约**（写在 `aiagent.h`）：只读、可重复调用、**不动棋盘**。因为面板会在对局中途被
  ...
  `test_match` **[2.11]** 对十六个 agent 逐个钉住（MCTS 的第一版就是被这条抓住的：它那次
  模拟要 `std::rand()`，于是拆成"0 次模拟（兜底路径，可报坐标）/ 1 次模拟（只报合法性）"）。
* **十六个 agent 全部实现**（2026-09：此前只有 DQN+MCTS 一个，其余七个显示"没有自检项"；
  随后又补上了新 agent PPO+MCTS-MLP，见 §21.6；再往后是 59e5233 还原版的两支
  —— `AGENT_SACAZ_OLD`（MLP）与 `AGENT_SACAZ_OLD_MOE`（TB 专家），两者是同一个类的
  两个骨干，见上面第五轮那段；最后是 Alpha-Beta 的三档弱等级 `AGENT_AB_L1/L2/L3`，
  它们各自的自检报告印的是**本档的实际深度** —— 报告里那句手抄的
  "(界面 AB_DEPTH = 4)" 就是被 [2.11] 这条钉子逼着删掉的，否则 L2 会在同一行里
  印出"深度 2 (界面 AB_DEPTH = 4)"）。
...
* **"全部模型自检"按钮**：把十六个 agent 的报告拼在一起 —— 编码/口径的差别只有横向对比
...
## 22. 人机对弈的状态机：应手线程的生命周期 + 沙漏那一行 agent 名（2026-09）

§13 记的是"思考过程可视化"，§15/§16 记的是"载入/保存时的沙漏窗"与"静默保存"。
这一节记的是**人机对弈这条状态机本身**：谁在什么时候唤醒黑方、终局之后它去哪、
以及沙漏卡片上"当前对战 agent"那一行到底显示谁。触发它的是一组用户报障
（问题编号 C27，完整记录见 `docs/issues_review.md` 零之二点二十九）。

### 22.1 报障：三句话，一个根因

| 用户的话 | 界面上实际发生的事 | 真正的机制 |
|---|---|---|
| 红方下第一个棋后黑方无限等待 | 状态条一直转，黑子永远不动 | 应手线程**已经不存在** |
| 选择黑方对弈 agent 失效 | 下拉框能选、`m_agentType` 也改了，但没人用它应手 | 同上 |
| 沙漏显示 agent 未加载 | 那一行写着 `(未选择 agent)` | ①`aiThinkingStarted` 再也不来；②按"开局"时 `resetToIdle()` 又把 agent 名清空 |

### 22.2 机制：终局有两条路，只有一条会把线程判死

`ChessBoard::process()` 是人机对弈里**唯一**的"黑方应手"线程（构造函数起一次）。
它原来的主循环是 `while (state != STATE_TERMINATE)`，而**终局恰恰就是把 `state`
置成 `STATE_TERMINATE`**：

| 谁走出终局 | 代码路径 | 工作线程的结局 |
|---|---|---|
| **玩家**走出最后一手 | `mousePressEvent` 里 `state = STATE_TERMINATE`，**没有 `wakeAll()`** | 还在 `condit.wait()` 里睡着，`reset()` 一叫就回来 ⇒ **不坏** |
| **AI**走出最后一手 | `process()` 里 `state = STATE_TERMINATE; … continue;` | 回到循环头、条件为假 ⇒ 线程函数 `return` ⇒ **永久消失** |

于是"红方自己赢/和"一切正常，"**黑方（AI）赢**"之后就永久卡死 —— 报障偏偏是后者。
这也解释了为什么上一轮把怀疑放在"保存权重持 `m_agentMutex`"上时怎么查都不对：
那条路已经在"只在退出时保存"那次改动里切断了，而这里**根本不是锁的问题，是线程没了**。

### 22.3 修法：唯一退出点 + 终局只"等"

* 主循环改 `for (;;)`；等待条件 `while (state != STATE_THINKING && !m_processStop)`。
* 终局仍然把 `state` 停在 `STATE_TERMINATE`（**棋盘继续拒收点击，语义不变**），
  但**线程留在等待里**；下一次 `reset()` + 红方落子把它叫醒，它接着服务新的一局。
* 新增 `std::atomic<bool> m_processStop`：**唯一**退出点，析构里在锁内置位 + `wakeAll()`。
  不加这一条，关窗时 `join()` 会一直等一个"不会再退出"的循环。
* **为什么不"在 `reset()` 里重建线程"**：`reset()` 可能落在"工作线程刚判定完终局、
  还没跑出循环"的那一瞬 —— 那样 `join()` / `std::thread` 赋值会等一个永远不会退出的循环。
  把**线程生命周期**与**对局生命周期**解耦才是正解（这一次的口径：线程活到对象析构为止）。

改动落在 `src/chessboard.h`（成员 + 注释）、`src/chessboard.cpp`（`process()` / `~ChessBoard()`）。

### 22.4 沙漏那一行：`m_configured` 与 `m_agent` 是两件事

`ThinkingIndicator` 卡片底部那行显示 `m_agent`，而它原来只在 `start()` 里赋值、
`resetToIdle()`（按"开局"）会清空 —— 于是"界面上选着 agent，卡片上写着未选择"。
现在分成两个来源：

| 成员 | 来源 | 什么时候显示 |
|---|---|---|
| `m_configured` | `MainWindow` 在"选 agent / 开局"时喂进来的**当前选中项** | 空闲时（`m_agent` 就取它） |
| `m_agent` | `start(agentName, …)`（`aiThinkingStarted` 带过来的） | 思考期间显示**正在想的那个** |

配套：`setConfiguredAgent()`、`displayedAgent()`（只读入口 —— 那一行是**画**出来的，
UIA 读不到，测试需要一个文本入口，与 `CurveChart::readoutText` 同一个思路）、以及
`resetToIdle()` 不再清空名字。界面侧在"构造 + 换 agent"两处喂值（`src/mainwindow.cpp`）。

### 22.5 优化方法：把一条 UI 状态机路径变成可自动化的断言

这条 bug 逃过了当时**全部**测试，原因是所有 `test_*` 走的都是
`humanTurnAiMoveForTest` / `matchAgents` —— **不经过 `process()` 的终局分支**。
新增 `test/probe_hvai_flow_main.cpp`（进 ctest，约 5–12 s）：

1. 用**合成鼠标事件**走 `mousePressEvent`（与玩家操作逐字相同，坐标抄
   `offsetX/offsetY=50, gridSize=60`）；
2. 摆一个"红方只剩帅、黑方车马"的局面，让 **AI 自己**把第一局结束掉
   （实测 5 手、结果 = 2 黑方胜，与报障现场一致）；
3. 按"开局"，红方再走第一步 → **黑方必须应手**；换对战 agent 之后同理；
4. 另加一节钉住 §22.4 的语义（`displayedAgent()` 在"设过 / 开局 / 思考中 / 思考结束"四种
   状态下各是什么）。

探针自己踩过四个坑，都是"自动化一条 UI 路径"的通用教训（详见 `issues_review.md`
零之二点二十九 §4）：

| 坑 | 现象 | 处理 |
|---|---|---|
| 终局弹窗是**模态 + `exec()` 嵌套循环** | 探针点不到它，第一局结束那一刻整个探针挂住 | 挂一个"关掉模态窗"的看门狗定时器（定时器在嵌套循环里照样触发）。⚠ 用 `SIGNAL/SLOT` 去断 ctor 里"新语法 + QueuedConnection"建的那条连接**断不掉**（实测返回 `false`） |
| 快照取晚了 | AB 在这个残局里一手只要 ~4 ms，先点击再取"黑方位置快照" ⇒ AI 已经落子，"等变化"永远等不到 | 快照在**点击之前**取 |
| `processEvents()` 不带 maxtime | 棋盘思考期间的 40 ms 动画定时器不停投递重绘 ⇒ "处理到没有待处理事件为止"基本不成立，每手从 16 ms 变成 ~5 s | 一律给 maxtime（探针里 10 ms） |
| 拿信号当判据 | 控制台探针没有界面那个常驻事件循环，"等待期间才建立的 `aiThinkingFinished` 连接"收不到那一次 emit | 判据改轮询盘面 —— 那也正是用户能看见的事实（"黑方到底动没动"） |

### 22.6 复现与实测

```bat
cmake --build <build> --target chess probe_hvai_flow
<build>\probe_hvai_flow.exe        :: 修后: 失败项 0 (约 5 s); 修前: 失败项 2
ctest --test-dir <build> -R probe_hvai_flow
```

修前 / 修后（同一台机器、同一份探针）：

```
[ OK ] 第一局: 收到 sendResult —— 这一局是**AI 的落子**结束的
    第一局: 共走 5 手结束, 结果 = 2 (1=红胜 2=黑胜 3=和)     <- 与报障"黑方胜利"一致
[ !! ] 第二局: 红方走完第一步后黑方应手了                    <- 修前: 15 s 超时
[ !! ] 换对弈 agent 之后: 黑方仍然应手                       <- 修前: 15 s 超时
==== 失败项: 2 ====                                          <- 修后: 失败项 0
```

顺带修掉的**假失败**：`test_match` [2.7i] 里 "到底线后不能再前进 (棋盘外)" 那条断言
拿 `Pos(-1,4)` 当"再前进"来测，而 `tryMoveTo` **不做棋盘边界校验**（x 从 0 变 -1 不是
"后退"、delta 恰好是 1 ⇒ 返回 true）—— `test/probe_pawn_rule_main.cpp` 的 [3] 节里
早就逐字写过这件事，探针躲过了、`test_match` 没躲过。现在两边用同一条正确形状
（"不能往回走 (x+1)" + "只能横走"）。整场 `test_match` 本机实测 **283 项断言 / 0 项失败**。

> **成本记录（2026-09-26 本机）**：`test_match` 整场约 **31 min**（`[2.7c]`/`[2.7d]` 的
> PPO/SAC 对局 + 530 MB 权重快照往返是大头），超过 ctest 里它自己的 `TIMEOUT 900`。
> 进程全程占满一个核、逐节推进（不是死锁），属于**用例成本**问题：要一次跑绿得把
> 上限提到 ~2400 s（本仓库上一次 600 → 900 也是同一种调整）。

---

## 23. "冻结"必须包含**权重不变**，以及让对手的棋进训练数据（2026-09，P0-b 收尾 + P1）

P0-a 把"这一场到底学不学"变成了显式选择（`MatchMode`），但它只解决了**归因的开关**，
没解决两件更基础的事。这一节是那两件的记录。

### 23.1 P0-b：评估模式下"冻结"曾经是假的（三条缝）

| 缝 | 形态 | 为什么之前的修法没盖住 | 修法 |
|---|---|---|---|
| ① 后台训练线程 | 它开机就在跑，每轮把 clone 的新权重**同步回主 agent** —— 而对局/人机用的就是这个主 agent | P0-b 第一版只在 `matchAgents` 里 `pause/resume`：**人机对战那条路不在那里**（用户把模式设成评估，跟 AI 下棋，AI 的权重照旧在局中被换掉）；**局与局之间**也一样（一场 4 局的评估，每局面对的对手都不同） | 训练线程自己每轮开头 + **写权重之前**各判一次 `isBackgroundTrainingPaused()`（= 显式暂停 ∨ 模式 ≠ 训练对局），非训练模式下**整体停摆**，并在锁内复查（在飞的那一轮也会被丢弃）。对局那一层的 `pause/resume` 保留为**屏障**（即使用户中途把模式切回去，这一场仍全程无后台写入） |
| ② A/B 共用实例 | `MATCH_EVAL` 只禁止 **B 自己**更新；而"每个 agent 类型只有一个常驻实例"（`m_sfXXX`）意味着 **A 与 B 同类型时它们是同一张网** ⇒ A 每学一次，B 当下用的权重就变一次 | 没有任何代码在管"B 用哪份权重"—— 冻结的是**学习**，不是**权重** | 评估模式开场取一份 B 的**权重快照**（必须在**对局循环之前**取：A 第一手就会更新那个共用实例），用快照单独建一个**冻结实例**，`decisionInstance()` 在"这一手是冻结方"时把决策路由到它；对局结束由 RAII 拆掉（留着的后果是**下一场**继续用上一场的快照下棋，而且没有任何读数能反映）。A/B 类型不同时不需要快照（两个对象，A 写不到 B）—— 报告里把这两种情况分开写清 |
| ③ 人机对战 | `aiThink` 完全绕过模式（P0-a 已修：AI 固定是"冻结的对手"），但那条路同样受①影响 | —— | ①修好之后自然覆盖（AI 是常驻实例，后台训练在评估模式下不动常驻实例） |

**可量的证据**（`test_match`）：

* `[2.7j]` 用**同一对 agent（PG vs PG）**跑两次，只差一个开关：
  * 对照（快照关）：常驻权重 `8987140 → 8987140` 字节但**内容不同**（`相同 = 0`）
    ⇒ **洞是真的** —— A 一学，B 用的那份权重当场就变；
  * 修好（快照开）：`B 用开场快照走了 10 手`，逐字节审计 `与快照逐字节相同 = 1`，
    而常驻实例**照样变了**（A 的学习没被一起冻掉）；
  * 报告新增两行："B 方冻结口径"（快照 / 独立实例 / 无可训练权重）与
    "逐字节审计"（可选的 `setFrozenWeightAuditEnabled`）。
* `[2.7k]` 在没有对局跑的情况下（等价于人机/局间）切模式：
  * 训练模式：后台训练**确实**改写了主 agent 的权重（正对照）→
  * 评估模式：权重**逐字节不变**，且"模式跳过了 N 轮"的计数在涨（= 确实是闸门拦下的，
    不是"训练碰巧没跑起来"）→
  * 切回训练模式：权重又能变（线程是活的，不是僵尸）。

**踩出来的两个"仪器"问题**（都值得记，因为它们是**假读数**而不是假代码）：

1. **第一版把"权重变了没有"建立在一份已经被写坏的权重上**。PG 有一条既有缺陷
   （`probe_pg_loss_nan`：参数变成非有限值且不复位），而 **nan 的字节表示是不变的** ——
   于是"两次保存逐字节相同"既可能是"没人动权重"，也可能是"权重早坏了，动也动不出差别"。
   现在测试里有两道防线：每次测量前把常驻 PG 装回**健康基线**（测试开头存的），以及一个
   **真的解码** base64 数 NaN/Inf 的判据（`nonFiniteWeightCount`）。
2. **第一版的"非有限值检测"是搜字符串**（`"nan"`/`"inf"`）—— 而权重是 base64 编码的
   **二进制**，这两个字母组合会随机出现在编码结果里：健康权重被判成"非有限"，
   真坏的文件反而可能判成"干净"。**判据必须与表示层对齐**，否则它只是又一个好看的读数。

### 23.2 P1：对手的棋进训练数据（`exploreAndTrain` 的对手参数）

**洞**：决策前的探索（`rolloutFromCurrent`）是**纯自对弈** —— "探索里对手那一半"也由
**学习方自己的策略**采样。于是无论对手是谁（哪怕选了 Alpha-Beta 当陪练），它学到的都是
"怎么跟自己下"；真实对手的着法**从未参与数据生成**。

**修法**：`AgentBase::exploreAndTrain(color, steps, opponent)` 多一个 `OpponentPolicy`
（`stepFor(turn) + name + budget`）。探索滚到"对手那一手"时改问**真实对手**：界面这一层把它
接到 `decideOnEnvRawLocked()` —— 也就是对手**对局时同一条决策代码**（同一套搜索/参数/实例
选择，含 P0-b 的冻结快照）。

**四条刻意的边界**（每一条都写进了注释与测试）：

1. **对手的着法不记成学习方的样本**。自对弈时"对手那一半"是学习方自己采样出来的，记成
   学习方的动作转移是 **on-policy** 的；换成真对手之后同一份记录会变成"off-policy 却打着
   on-policy 标签" —— 对 REINFORCE/PPO 是实打实的偏差（冒充进来的动作从来没以那个概率被
   采样过）。所以它**只推进局面**：学习方自己的转移落在"真对手应手之后的局面"上，回报也
   带着对手的影响，而这些样本仍然是干净的 on-policy 数据。
   * **唯一的例外是 EVAB**：它的样本是"局面 → 价值"（`encodeCanonical(turn, …)` 相对走子方），
     与"谁选了这一步"无关 ⇒ 对手的着法在那里是纯粹的数据增广，照记不误。
2. **对手被问到时不许学习**。问一次对手 = 走一次它的决策协议，而 SAC+AZ 的 `selectMove`
   里带着 `learnFromSearch` 那条更新路径 —— 不拦住的话"问对手一手"会让对手在**学习方的
   探索过程中**悄悄更新一次（评估模式下 B 当场就不是冻结的了），而且会无限递归。
   拦住有两层：`preTrainThenDecide` 在查询期间直接返回；`searchLearningEnabled()` 在查询
   期间返回 false（掩码走它）。两层都计 `opponentQueryLearnBlockedCount()`。
3. **只在对弈里有对手**：人机对战没有"对手的策略"（人是人），那条路保持自对弈。
4. **默认关闭 + 手数预算**：问一次对手 = 一次完整决策（SAC+AZ 实测 2.3 s/手），而一次探索
   要滚几十手。默认 `budget = 0`（行为与改动前**逐字相同**），界面打开时给 1
   （= 只问"对手对学习方第一步的应手"）。
   ⚠ 口径提醒写进了界面提示：打开后价值/策略目标**条件化于这个对手**，混着不同对手训练时
   `V(s)` 学的是平均值（对手特征没有进状态）—— 所以它适合"专门练一个对手"，不适合常开。

**证据**：

* `test_pretrain`（探索内部三条语义，用桩 agent + 桩对手，全部确定性）：
  ```
  无对手参数 : 样本 8 条, 问对手 0 次
  有对手参数 : 样本 6 条, 问对手 2 次, 用上 2 手, 回退 0 次     <- 样本少掉"对手手数"
  对手全给无效: 样本 8 条, 问对手 4 次, 用上 0 手, 回退 4 次     <- 完全回退, 轨迹与"没开"逐字节相同
  ```
  其中"轨迹与不开对手时不同"是**对手的着法真的被走出来**的证明（桩的 `pick` 走第一个合法
  走法，对手走最后一个 ⇒ 一定不同）；"样本数少掉对手手数"是"不冒充成自己的动作"的可测形式。
* `test_match [2.7l]`（对弈里）：开关关着时**一次都不问**；打开后**真的问了**（计数 > 0）、
  对手的着法**全部对得上**合法集、被问到的 B（SAC+AZ）**一次都没上报损失**且
  "本来会发生的学习"被拦下的计数 > 0、A 照常学、探索说明里印出"对手(X)应手 N/M 手"。

### 23.3 顺带修掉 / 明确不做

* **顺带**：`saveCurrentAgentModel` 的类型分派收敛到 `saveWeightsOf()`（原来"存档 / 取快照 /
  审计"三处各有一份 switch，"以后新增 agent 类型时漏掉一处"就是静默失效）；
  `aiThinkForAgentRaw` 的加锁粒度从"每个 case 各拿一次"提到**整段决策**（P1 的"问对手一手"
  发生在学习方决策的锁内，那里再进一次同一个 case 就是非递归锁的自锁死锁 —— 所以决策体被
  抽成 `decideOnEnvRawLocked()`，契约写在函数头）。
* **明确不做**（连同理由，免得以后被读成遗漏）：
  * **把对手的着法当样本收进来**（而不是只推进局面）：那要求"给网络对手特征"或按 off-policy
    方法处理，否则价值目标不可辨识 —— 见 `chessboard.h` 里 `AGENT_ALPHABETA` 的注释。
  * **P1 的"对手模型"只在一手深度上条件化**（budget 默认 1）：更深要按对手的完整搜索逐手付费，
    那是成本问题而不是口径问题，旋钮已经给了。
  * **对局模式在下拉框里不持久化**（重启回到"训练对局"）：它是"这一场"的语义，不是偏好设置。
