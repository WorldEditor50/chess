...
### 16.3 验证脚本自己踩的三个坑（都真实发生过，记下来免得再犯）

这三条都不是被测代码的问题，而是"检查写得不对"，而且**错误的检查比没有检查更坏**：
它会给出一个看起来很像结论的答案。

**(1) 后台保存还没写完就去读列表 → 假失败。**
静默保存是后台线程，稀疏 MoE 那 438 MB 要一两秒（实测 1983 ms）。第一版脚本在下棋结束的
列表，于是 `silent weight save line = False` —— 代码是对的，脚本错了。现在改成**轮询等
它出现**（最多 60 s，`All-ListItems()` 每 500 ms 重读一次）。
