...
### (2) 未修（本轮明确留白，全部列在 training_optimization.md §7）

1. **最重要的一条：奖励/评估改动还没有棋力验证。** 本轮的结论全在"信号层"（目标分布、
   尺度不变量、平移性、代价），**等时间对局的胜率对照还没跑**。下一步就是它。
...
5. **搜索侧**：策略头只算合法列（估算 3.0 MB → 0.9 MB/模拟）、静止搜索、置换表 + 子树复用
   （PPO+MCTS 至今**没有置换表**，而且每 ply `nodes.clear()`）、top-K/渐进放宽。
