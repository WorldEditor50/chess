...
| **C25** | **SAC 的 critic 训练 20 局后什么游戏信息都没有（"|Q| 趴在 0.035"的真因）**：|Q| 的三个实测值（0.031 / 2.04 / 5.6）看着像三种健康度，其实是**同一个与棋局无关的常数**在不同 α 与钳位下的样子。链条：`replaceTargetIter=64` × Polyak `tau=1e-3` ⇒ 一次 20 局只把**目标网**从随机初始化挪动 2~4%（实测 `|Q_target| = 0.072~0.097`，随机初始化尺度是 0.06~0.074）⇒ 自举项 `V(s') = E[minQ_target] + α·H` 里的游戏部分恒为 0、**100~273% 由熵项 α·H 构成** ⇒ `y ≈ r − γαH`，|Q| 由 αH 与 `clampTarget` 决定（2.04/3.11/4.02 正好贴在钳位上，不夹就漂到 10~11.6，α 落到下界时塌到 0.03）；而 α 往哪跑由"目标熵可不可达"决定 —— `H̄ = 熵比·log(着法数)` 却作用在**合法槽位**上（128 槽哈希碰撞，`H ≤ log(槽位) < log(着法)`），熵比 0.98 时实测 `H<H̄` 占 58~75%（α 单调爬到上界），熵比 0.5 时只占 0.5%（α 塌到下界）。策略侧同样没有方向（`−min Q_a` 在各动作上是常数）⇒ 归一化策略熵实测 **0.986~1.000**（策略 = 均匀先验） | **未修（只定位 + 量化）**：新增训练中诊断（`TrainDiag`：夹前 y 分布/夹住比例、V 的两项分解、α 轨迹、H 与两种 H̄、`Qspread`）+ 目标网探针 + 三个实验开关（`entropyInTarget` / `entropySlotsAsLegal` / `valueScale`）。**两个"看起来该修"的开关实测都让棋力显著变差**（拿掉熵项 65.0%→27.5%，p≈1e-06；换分母 65.0%→26.7%，p≈1e-07）——四个缺陷是**串联**的，修一个不够。建议顺序 F1 目标网同步率 → F2 熵项出目标 → F3 目标熵分母 → F4 策略方向。见 [`docs/sac_critic_diagnosis_2026_09.md`](sac_critic_diagnosis_2026_09.md) |
| **C26** | **界面奖励曲线与 agent 的学习口径差 10 倍**：用户拿导出的 CSV 读出"材质比赢棋重要 3.5 倍"，而 agent 学的是 0.35 : 1。曲线取的是 `Chess::moveForward` 的 `totalReward`（**材质 ×1**、黑方视角记账），而 agent 学的是自己的 `computeReward()`（**材质 ×0.1** + 每步代价）+ 终局 | 同一局里两个口径差 **10 倍**（实测 CSV 里奖励最大值 4.5 = 3.5+1.0，只有曲线口径能到）；"曲线上的比例"因此永远解释不了"学习信号的比例" | **已修复**：奖励曲线改成**学习口径**（`AgentBase::hasLearningReward/learningStepReward/learningTerminalReward` + `ChessBoard::RewardAccounting` 两种口径都记账），曲线名与逐局明细标出 `[学习口径]`/`[引擎口径]`（纯搜索 agent 保留引擎口径并标注），CSV 表头随曲线名写明口径。`test_match` **[2.19]** 断言钉住"学习口径逐位等于 agent 自己的函数"与"两本账的精确换算 `学习即时 = 0.1×引擎即时 − 0.001×手数`"。见"零之二点二十八" |
| **C27** | **人机对弈"黑方胜利之后, 红方下第一个棋黑方无限等待"（连带"选择对战 agent 失效"与"沙漏显示 agent 未加载"）**：报障三句话是**同一个根因**。`ChessBoard::process()` 的主循环写成 `while (state != STATE_TERMINATE)`, 而**终局恰恰就是把 state 置成 `STATE_TERMINATE`** —— 于是"**AI 自己**走出将杀/困毙/判和"的那一局结束时, 那个 `continue` 之后循环条件立刻为假, 线程函数 `return`, **人机对弈的应手线程永久消失**。之后按"开局"只把 state 改回 IDEL（没有任何人再起线程）, 红方落子 → `state=THINKING + wakeAll` 却**没有等待者** ⇒ 黑方不动; 换对战 agent 也没人去用它应手; 而沙漏再收不到 `aiThinkingStarted`, 加上 `resetToIdle()` 会清空 agent 名 ⇒ 那一行显示 `(未选择 agent)`（用户读成"agent 未加载"）。**玩家自己走出终局那一支不会**触发（那条路上没有 wakeAll, 线程还在 `condit.wait` 里睡着）—— 这正解释了报障为什么偏偏是"黑方胜利后" | **已修复**：主循环改 `for (;;)`, 终局只把 state 停在 `STATE_TERMINATE`（棋盘继续拒收点击, 这仍然是对的）, **线程留在等待里**; 新增 `m_processStop`（析构里置位 + wakeAll）作为**唯一**退出点, 把"线程该不该活着"与"这一局结没结束"解耦。沙漏那一行改成显示**界面当前选中的对战 agent**（新增 `ThinkingIndicator::setConfiguredAgent`, `resetToIdle()` 不再清空它）。新增回归测试 **`probe_hvai_flow`**（进 ctest, 12 s）: 真实点击路径连打两局 —— 第一局**由 AI 的落子结束**（结果 2 = 黑方胜, 与报障现场一致）, 再按"开局"走红方第一步, 黑方必须应手; 换 agent 之后同理。修前 `失败项: 2`（两处都超时）, 修后 `失败项: 0`。见"零之二点二十九" |
...
## 零之二点二十八、C25/C26 详录：critic 的"尺度"不是棋力，奖励曲线换了口径（2026-09）

### 1. 为什么"|Q| 趴在 0.035"这件事比它看起来严重

上一节（零之二点二十七）把 `|Q| ≈ 0.034` 记成"α 口径把 critic 弄死了"，并把
"改回 0.98/1e-3 后 `|Q| = 2.01`"当成**修好了**。第二轮把这件事量到底之后发现：
**两个 |Q| 值都是"没有游戏信息"，区别只是被推到的常数不同**。链条与实测（全部读数见
[`docs/sac_critic_diagnosis_2026_09.md`](sac_critic_diagnosis_2026_09.md)）：

1. **软备份用的是目标网，而目标网几乎没动**：`replaceTargetIter=64` × Polyak `tau=1e-3`
   ⇒ 一次 20 局（1300~2900 次 learn）只移动 **2.0~4.4%**。实测 `|Q_target| = 0.072~0.097`
   —— 与"随机初始化"（0.06~0.074）不可区分，**在所有配置里都一样**（α 0.5 与 0.98、
   夹与不夹、还原版与最新实现）。
2. **于是 TD 目标几乎就是熵项**：`V(s') = E[minQ_target] + α·H`，实测
   `E[minQ] = −0.045~−0.057`（随机尺度）、`α·H` 占 100~273%。`y = r − γV` ⇒
   `|Q|` ∝ `αH`，被 `clampTarget` 截断：**2.043 / 3.106 / 4.020 正好是钳位 2/3/4**
   （不夹则漂到 10~11.6；α 落到下界 0.02 时塌到 0.03）。
3. **α 的方向由"目标熵可不可达"决定**：α 的梯度是 `H − H̄`，而 `H̄ = 熵比·log(着法数)`
   作用在**合法槽位**上（128 槽哈希碰撞 ⇒ `H ≤ log(槽位) < log(着法)`）。
   实测熵比 0.98 时 `H<H̄` 占 **58~75%**（α 从 0.2 单调爬到 2.6，还在涨），
   熵比 0.5 时只占 **0.5%**（α 一局内就掉到下界）。**同一批样本只把分母换成槽位数，
   这两个比例就翻成 1~6%**。
4. **策略侧也没有方向**：`g_a = α(log π_a + 1) − min Q_a`，而 `min Q_a` 在各动作上是同一个
   常数 ⇒ 策略只被熵项推向均匀。实测归一化策略熵 **0.986~1.000**。

**结论**：这个 agent 训练 20 局之后仍然是"**均匀先验 + 256 次模拟的 PUCT + 一个恒定的 Q**"；
它与"完全不训练"的对照（70.0%）在噪声内不可区分 ⇒ **"被将死 14/20"与"|Q| = 0.035"是
同一个根因的两个症状**，不是"|Q| 小 ⇒ 棋力差"的因果。

**两个"看起来该修"的开关都做了，都更差**（这也解释了为什么历轮"改进"都没提高棋力）：

| 开关 | 作用 | 得分率（3 seed × 20 局） | 与默认的 Fisher |
|---|---|---|---|
| 默认 | — | 65.0%（23-5-32） | — |
| `--entropy-in-target=0` | 熵项不进自举目标（`|Q|` 2.04 → 0.04，符合预期） | **27.5%**（13-40-7） | p = 1.0e-06（更差） |
| `--entropy-slots` | 目标熵的分母换成槽位数（α 立刻从"爬到上界"变成"掉到下界"） | **26.7%**（9-37-14） | p = 1.4e-07（更差） |

四个缺陷（F1 目标网同步率 / F2 熵项进目标 / F3 目标熵分母 / F4 策略无方向）是**串联**的，
单独修任何一个都不会有收益。建议顺序与判据见新文档 §9。

### 2. C26：界面奖励曲线换口径

* **取数**：`ChessBoard::playMatchGame` 现在在 `moveForward` **之前**取 agent 自己的
  `computeReward()`（必须在落子前取 —— 落子后 `alive=false`，吃子奖励会恒为 0），
  并在终局问 agent 要 `terminalReward()`（SAC 塑形开着时是 ±(1+败方材质/3.5)；
  取终局值前把 agent 的试走棋盘 `env` 同步到**终局局面**，否则它数的是上一手）。
* **两本账都记**（`ChessBoard::RewardAccounting`）：引擎口径恒记、学习口径在 agent 有口径时记。
  这样"换口径"破坏历史可比性的代价是可解释的，而且断言有了参照：
  **`学习即时 = 0.1 × 引擎即时 − 0.001 × 该方手数`**（与棋局无关，逐手可验）。
* **标注**：曲线名（`agentA [学习口径]` / `[引擎口径]`）、图表标题、逐局明细
  （`奖励 A=+0.12[学习口径] B=+3.50[引擎口径]`）、导出的 CSV 表头都写明口径 ——
  两个口径差 10 倍，**同一张图上混着两种口径的线时不可直接比大小**。
* **回归钉**：`test_match [2.19]`（口径表与真实对象一致 / 学习口径逐位等于 agent 自己的函数 /
  曲线值 == 学习口径账 / 两本账的精确换算 / 纯搜索对手不替换口径）。

---

## 零之二点二十九、用户报障："黑方胜利之后，红方下第一个棋黑方无限等待"（2026-09）

> 报障原文（用户）：**"人机对弈时黑方胜利后，沙漏显示 agent 未加载，选择黑方对弈
> agent 失效，红方下第一个棋后黑方无限等待"**。
> 三句话是**同一个根因**，而且它不在"保存权重"上 —— 上一轮怀疑的那条路（保存持
> `m_agentMutex` 写 530 MB 把决策挡在锁上）已经在"只在退出时保存"那次改动里被切断了，
> 报障照旧复现。

### 1. 先把报障的三句话翻成"界面上的事实"

| 用户的话 | 界面上实际发生的事 | 代码里对应什么 |
|---|---|---|
| 红方下第一个棋后黑方无限等待 | `state` 停在 `STATE_THINKING`，棋盘顶部状态条一直转，黑子永远不动 | `process()` 的应手线程**已经不存在了** |
| 选择黑方对弈 agent 失效 | 下拉框能选、也能改 `m_agentType`，但没有任何人去用新 agent | 同上（没人应手，换谁都一样） |
| 沙漏显示 agent 未加载 | 沙漏卡片下方那一行显示 `(未选择 agent)` | `aiThinkingStarted` 再也不来；按"开局"时 `resetToIdle()` 又把 agent 名清空 |

三条都指向同一件事：**"黑方应手"这条线程在某一刻消失了，而且再也没回来。**

### 2. 根因：`process()` 把"线程该不该活着"和"这一局结没结束"写成了同一个条件

```cpp
while (state != STATE_TERMINATE) {        // ← 原来的主循环
    ...
    if (result != Chess::RESULT_ONGOING) {
        state = STATE_TERMINATE;          // ← 终局
        emit aiThinkingStopped();
        emit sendResult(result);
        continue;                         // ← 回到循环头: 条件立刻为假 -> 线程 return
    }
}
```

于是只要"**AI 自己的落子**结束了一局"（将杀 / 困毙 / 判和），这个线程函数就 `return`
了。之后：

* `reset()`（按"开局"）只把 `state` 改回 `STATE_IDEL` —— **没有任何人重新起线程**
  （`processThread` 只在构造函数里起过一次）；
* 红方落子 → `state = STATE_THINKING; condit.wakeAll();` —— **没有等待者**；
* 沙漏 / 状态条 / `aiThinkFinished` 全都不会再被触发。

**为什么报障偏偏是"黑方胜利后"**：终局有两条完全不同的路 ——

| 谁走出终局 | 代码路径 | 工作线程 |
|---|---|---|
| **玩家**走出最后一手 | `mousePressEvent` 里 `state = STATE_TERMINATE`，**没有 wakeAll** | 还在 `condit.wait()` 里睡着，`reset()` 一叫就回来 ⇒ **不会坏** |
| **AI**走出最后一手 | `process()` 里 `state = STATE_TERMINATE; … continue;` | 循环条件为假 ⇒ **线程退出** ⇒ 之后全坏 |

所以"红方自己赢/和"的时候一切正常，"黑方（AI）赢"之后就永久卡死 —— 用户报的是后者。

### 3. 修法：唯一的退出点 + 终局只"等"

* 主循环改 `for (;;)`；等待条件改成 `while (state != STATE_THINKING && !m_processStop)`。
* 终局仍然把 `state` 停在 `STATE_TERMINATE`（**棋盘继续拒收点击，语义不变**），
  但**线程留在等待里**；下一次 `reset()` + 红方落子会把它叫醒，它接着服务新的一局。
* 新增 `std::atomic<bool> m_processStop`：**唯一**退出点，析构里在锁内置位 + `wakeAll()`。
  ⚠ 不加这一条，关窗时 `join()` 会一直等一个"不会再退出"的循环。
* 为什么不"在 `reset()` 里重建线程"：`reset()` 可能落在"工作线程刚判定完终局、还没跑出
  循环"的那一瞬 —— 那样 `join()`（或 `std::thread` 赋值）会等一个永远不会退出的循环。
  **把线程生命周期与对局生命周期解耦，才是这件事的正解。**

沙漏那一行同时改了语义（用户报障的第三句）：新增
`ThinkingIndicator::setConfiguredAgent()`，界面在"选 agent / 开局"时把当前选中项喂进去，
`resetToIdle()`（按"开局"）**不再清空**它 —— 空闲时显示的就是"要跟你下棋的那个 agent"，
而不是"未选择 agent"。配套一个只读入口 `displayedAgent()`（那一行是**画**出来的，
UIA 读不到，测试需要一个文本入口，与 `CurveChart::readoutText` 同一个思路）。

### 4. 回归：`probe_hvai_flow`（进 ctest，约 12 s）

**为什么必须是"真实点击路径"**：这条 bug 需要"一局由 AI 自己结束"才会暴露，而当时
**没有任何测试覆盖它** —— 所有 `test_*` 走的都是 `humanTurnAiMoveForTest` /
`matchAgents`，根本不经过 `process()` 的终局分支。所以新增
`test/probe_hvai_flow_main.cpp`：用合成鼠标事件走 `mousePressEvent`（与玩家操作逐字相同），
摆一个"红方只剩帅、黑方车马"的局面，让 **AI 自己**把第一局结束掉，然后按"开局"再走第一步。
...
修后同一份探针：三节（含沙漏那一行的语义）全部 `OK`，`失败项: 0`。

**探针自己踩过的四个坑**（都写进了注释，免得下一个人重踩）：

1. **终局弹窗是模态 + `exec()` 嵌套事件循环**：探针点不到它，一弹出来就把整个探针挂住。
   用 `QObject::disconnect(&board, SIGNAL(sendResult(int)), &board, SLOT(checkGameOver(int)))`
   **断不掉** ctor 里用"新语法 + `QueuedConnection`"建的那条连接（实测返回 `false`），
   只能挂一个"关掉模态窗"的看门狗定时器（定时器在嵌套循环里照样触发）。
2. **快照要在点击之前取**：AB 在这个残局里一步只要 ~4 ms，而探针的"玩家"是瞬时落子 ——
   先点击再取"黑方位置快照"，AI 那一步可能已经落地了，"等变化"于是永远等不到
   （表现成假超时，第一版就是这么误判的）。
3. **`QApplication::processEvents()` 不带 maxtime 会转很久**：棋盘在思考期间有 40 ms 的
   动画定时器不停投递重绘事件，于是"处理到没有待处理事件为止"这个语义基本不成立 ——
   实测每手从 16 ms 变成 ~5 s，80 手的一局像卡死。必须给 maxtime。
4. **"待发信号"不能拿来做探针的判据**：本探针是控制台程序、没有界面那个常驻事件循环，
   实测在"等待期间才建立的 `aiThinkFinished` 连接"上收不到那一次 emit
   （同一时刻常驻连接能收到）。判据改成轮询盘面 —— 那也正是用户能看见的事实
   （"黑方到底动没动"）。界面用的是常驻连接，不受这条影响。
...
18. **`test_match` 的 ctest 超时上限与本机实测成本不匹配**（2026-09-26 实测）：整场
    **283 项断言 / 0 项失败**，但需要约 **31 min**，而 `CMakeLists.txt` 里给它的是
    `TIMEOUT 900`。进程全程占满一个核、逐节推进（**不是死锁**；`[2.7c]` 的权重快照
    往返与 `[2.7d]` 的 PPO/SAC 对局是大头），所以这是**用例成本**问题而不是正确性
    问题：ctest 一次跑下来会看到 `test_match (Timeout)` 红着。修法两种，取其一：
    把上限提到 ~2400 s（本仓库上一次 600 → 900 就是同一种按实测调整），或把
    `[2.7c]`/`[2.7d]` 拆成独立目标/缩短每局手数（会削弱它们的断言强度）。
    **`probe_hvai_flow`（约 5 s）与其余 12 个条目在同一轮实测全部通过。**
