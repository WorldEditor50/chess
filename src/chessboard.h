...
public:
     ...
     * 产物, 一次都没存过权重时它里面就是空的, 自检面板照实写"启动时未载入 (扫描没命中)"
     * 是对的。但这句话原来**只出现在日志与自检面板里**, 而用户按"开局"时看不到它们 ——
     * 于是"没有模型"这件事只表现为"AI 下得很差/像随机", 没有任何一条人能看见的提示。
     *
     * 这个方法给界面用: 返回**一行**纯文本 (命中几组 / 缺哪几组)。
     * 只在**启动加载完成之后**有意义 (之前 s_weightPaths 还没扫完, 会误报成"全缺")。
     */
    std::string weightLoadSummary() const;
    /*
     * 同一件事的**详细说明** (多行): 为什么没有权重、怎么才会产生、去哪儿看每个文件在不在。
     * 与 weightLoadSummary 分开的理由: 那一行要放进 QListWidget 的**单行**里, 而这里
     * 要放进它的 tooltip —— 单行里塞换行会被压平成一团, 反而看不清。
     */
    std::string weightLoadHint() const;
