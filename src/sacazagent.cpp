...
/*
 * 界面上的哪一支。基类按骨干区分 (同一个类被 AGENT_SACAZ 与 AGENT_SACAZ_MOE 复用),
 * 派生类 (SACAZLegacyAgent) 覆盖它。见头文件里的说明。
 */
...
/* 当前 SAC 的权重前缀。派生类 (59e5233 复现版) 另有一个 —— 见头文件的说明。 */
...
std::string SACAZAgent::selfCheckReport() const
{
    ...
    /* ---- 0. 本实例是**哪一支** ----
       必须放在最前面: 这个类现在有两个身份来源 ——
         ...
         * 类 (AGENT_SACAZ_OLD 是**派生类** SACAZLegacyAgent, 口径与当前不同)。
