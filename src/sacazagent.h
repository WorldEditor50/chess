...
public:
    ...
    */
    ...
    /*
       ...
       修法: 把 tau 变成可调成员 (`targetTau`), 并把**默认值按"一次会话几千步"重新定标**
       (逐臂 200 局 x 4 种子的配对实测见 docs/sac_critic_diagnosis_2026_09.md §13):
         * `targetTau = 1.0` + `replaceTargetIter = 256` (**当前默认 = 硬拷贝**) ——
           目标网最多陈旧 256 步。实测目标网 |Q| 从 0.113 抬到 **1.547**、
           与在线网 (1.452) 同量级, `E[minQ]` 从 −0.068 变到 **−1.663**;
           得分 65.0% → **67.3%**、决胜局胜率 75.4% → **80.0%** (p = 0.43, 方向为正但不显著)。
         * `targetTau = 0.01` + `replaceTargetIter = 8` (Polyak, 滞后 ~800 步) 同样能让
           目标网与在线网对齐 (|Q| 2.013 / 2.021), 得分 65.3% (p = 0.88) ——
           与硬拷贝的差别在噪声内 (p = 0.36), 选硬拷贝是因为它的滞后更短、最好解释。
         * `targetTau = 0.05` + 8 步: 62.0% (更快反而更差, p = 0.55)。
         * 老口径 (`targetTau=1e-3` + `replaceTargetIter=64`) 仍然可复现, 见
           `bench_sac_learn --target-tau=0.001 --target-iter=64`。
       **注意**: 这一改修的是**已被证明的机制缺陷** (自举项里没有游戏信息), 棋力收益
       方向为正但 200 局/臂下**不显著** —— 别把它读成"提升了 2 个点"。
       ================================================================
    */
