    ...
    */
    ...
    /*
       ...
       **骨干**变 —— 只写死一个字符串就会让两支的读数记到同一本账上 (本仓库踩过一次:
       ...
       [2026-09 修正] "随骨干变"**不等于**"只有两个骨干有名字": 第一版只给 Mlp 与
       SparseMoeTb 写了分支, 其余骨干统一返回一句 `bench/测试构造 (界面不为它建实例)` ——
       于是 test_sacaz [14] 拿另外两个骨干 (Mlp / SparseMoeMlp) 构造实例时, "标签里有
       59e5233" 这条断言在 SparseMoeMlp 上如实失败。**四件事**现在一起成立:
         (a) 每个骨干的标签都含 `59e5233` —— "还原口径"这个身份**不随骨干丢**;
         (b) 每个骨干的标签**两两不同**, 且写出自己的变体后缀 (MLP / MoE(TB) / MoE(MLP) /
             稠密(TB)) —— 面板上分得清是哪一支;
         (c) 界面上的两个骨干 (Mlp / SparseMoeTb) 的标签里带自己的**界面类型名**
             (AGENT_SACAZ_OLD / AGENT_SACAZ_OLD_MOE), 把"标签"与"界面类型"钉在一起;
         (d) 不是界面类型的骨干 (只被 test/bench 构造) 在标签里**明说**这一点, 免得有人
             以为面板上能选出它。
       判据与权重前缀一样是**单一来源**: 实例版本 guiAgentLabel() 只转发到
       guiAgentLabelFor(backbone), 测试用静态版本就能覆盖到**四个**骨干 ——
       不必为了检查 SparseMoeTb 的标签去构造一个 TB 实例 (那一支建网 + 拷贝权重很贵)。
    */
    const char *guiAgentLabel() const;
    /* 同一判据的**静态**形态 (按骨干取标签; 面板与测试不必先建实例) */
    static const char *guiAgentLabelFor(Backbone b);
